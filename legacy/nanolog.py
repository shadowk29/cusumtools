...
class LogGUI(tk.Frame):
    def __init__(self, parent):
        ...
        col = 0
        for key, val in self.prep_dict.items():
        ...
        framecol = 0
        for frame, widgets in self.entry_dict.items():
        ...
        framecol = 0
        for frame, widgets in self.mode_dict.items():
        ...
        self.set_date()
        self.run_log_path=''
        for frame, widgets in self.failure_frames.items():
    ...
    def load_standard(self):
        for frame, widgets in self.standard_dict.items():
            for key, val in widgets.items():
                self.entry_strings[frame][key].set(val)
        for key, val in self.standard_prep_dict.items():
    ...
    def read_run_log(self):
        self.run_log_path = tkinter.filedialog.askopenfilename(initialdir='C:/Data/')
    ...
    def grey_outcome(self):
        if self.outcome.get() == 0:
            for frame, widgets in self.failure_frames.items():
                self.disable_frame(widgets)
                for key, val in self.checkvars[frame].items():
                    val.set(0)           
    ...
    def verify(self):
        ...
        if self.outcome.get() == -1:
            ...
            total = 0
            for key, val in self.checkvars['Intervention'].items():
                total += val.get()
            if total == 0:
            ...
            total = 0
            for key, val in self.checkvars['Failure'].items():
                total += val.get()
            if total == 0:
